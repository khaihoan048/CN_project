import React from "react";
import {Avatar, Conversation} from "@chatscope/chat-ui-kit-react";


const avatarIco =
    "data:image/png;base64,";

export default class Renderer {
    constructor() {
        this.friendList = require("../data_sample/friends.json");
    }

    renderConversations() {
        return this.friendList.map((friend) =>
            (<Conversation name={friend.name} key = {friend.name} userName = {friend.userName}>
                <Avatar src={avatarIco} name={friend.name} status="available"/>
            </Conversation>));
    }
}
